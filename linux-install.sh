...
installapp() {
    ...
    colorEcho ${GREEN} "开始安装MinerProxy-V-${VERSION}"
    ...
    wget -P $PATH_KT "${DOWNLOAD_HOST}/MinerProxy_v${VERSION}_linux" -O "${PATH_KT}/${PATH_EXEC}" 1>/dev/null

    filterResult $? "拉取程序 MinerProxy_v${VERSION}_linux"
...
echo "-------------------------------------------------------"
colorEcho ${GREEN} "欢迎使用MinerProxy安装工具, 请输入操作号继续。"
